...
public class VisAppoiViewImpl implements VisAppoiView{
	ControllerVisAppoiObserver observer;
	final JFrame frame = new JFrame();
	private static final String TITLE_FRAME = "Visualizzazione appuntamenti";
	  
	public static final int SPACE = 5;
	public static final String TITLE = "Visualizzazione appuntamenti per una certa data";

	String[] titleTable = {"Dottore", "Paziente","Motivo"};
	
	private final JLabel title = new JLabel(TITLE);
	    
	private final JTextField textData = new JTextField("dd-mm-yyyy", 12); //written that will be removed when clicked
	...
	public VisAppoiViewImpl(ControllerVisAppoiObserver observer) {
        ...
        visualBtn.addActionListener(e ->{
        	...
        	if(date.isEmpty()) {
        		...
        		observer.visualizeData(date.get());
        	}
	...
	@Override
	public void updateTableAppoitnment(List<Appointment> appointList) {
		final DefaultTableModel model = (DefaultTableModel) table.getModel();
	    table.setModel(this.getModel(appointList));
	    model.fireTableDataChanged();
	}
	
	private DefaultTableModel getModel(final Collection<Appointment> patAppointSet) {
        final int row = patAppointSet.size();
        final String[] columnNames = titleTable;
        Object[][] data = new Object[row][columnNames.length];
        int i = 0;
        for (final var elem : patAppointSet) {
            data[i][0] = elem.getDoctor();
            data[i][1] = elem.getPatient();
            data[i][2] = elem.getMotivation();
            i++;
        }
