...
public class VisReportViewImpl implements VisReportView{

	private ControllerVisReportObserver observer;
	final JFrame frame = new JFrame();
	private static final String TITLE_FRAME = "Visualizzazione referti";
	  
	public static final int SPACE = 5;
	public static final String TITLE = "Visualizzazione di un referte paziente";

	String[] titleTable = {"Data", "Descrizione motivo", "Descrizione risultato"};
	...
	public VisReportViewImpl(ControllerVisReportObserver observer){
	...
	@Override
	public void updateTableReport(List<Report> reportList) {
		final DefaultTableModel model = (DefaultTableModel) table.getModel();
	    table.setModel(this.getModel(reportList));
	    model.fireTableDataChanged();
	}
	private DefaultTableModel getModel(final Collection<Report> reportSet) {
        final int row = reportSet.size();
        final String[] columnNames = titleTable;
        Object[][] data = new Object[row][columnNames.length];
        int i = 0;
        for (final var elem : reportSet) {
            data[i][0] = elem.getDate();
            data[i][1] = elem.getMotivo();
            data[i][2] = elem.getRisultato();
            i++;
        }
