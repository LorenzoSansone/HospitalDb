...
public class VisNextAppViewImpl implements VisNextAppView{
	private ControllerVisNextAppObserver observer;
	final JFrame frame = new JFrame();
	private static final String TITLE_FRAME = "Visualizzazione prossimo appuntamento";
	  
	public static final int SPACE = 5;
	public static final String TITLE = "Visualizzazione del prossimo appuntamento di un paziente";

	String[] titleTable = {"Data", "Dottore", "Motivo"};
	...
	public VisNextAppViewImpl(ControllerVisNextAppObserver observer) {
	...
	@Override
	public void updateTableNextApp(Appointment appointment) {
		final DefaultTableModel model = (DefaultTableModel) table.getModel();
	    table.setModel(this.getModel(appointment));
	    model.fireTableDataChanged();
		
	}
	private DefaultTableModel getModel(final Appointment appointment) {
        final int row = 1;
        final String[] columnNames = titleTable;
        Object[][] data = new Object[row][columnNames.length];
        int i = 0;
        data[i][0] = appointment.getDate();
        data[i][1] = appointment.getDoctor();
        data[i][2] = appointment.getMotivation();
        i++;
